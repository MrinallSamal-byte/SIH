    ...
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <Building className="h-6 w-6 text-zinc-800 dark:text-white" />
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-xl border border-zinc-200 bg-white px-3.5 py-2 text-xs font-bold text-zinc-700 transition hover:bg-zinc-50 disabled:opacity-50 dark:border-white/[0.1] dark:bg-[#222222] dark:text-white shadow-xs cursor-pointer"
        >
      ...
      <div className="mt-4 space-y-3 rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-xs dark:border-white/[0.08] dark:bg-[#1a1a1a]">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-zinc-200 bg-white pl-10 pr-3.5 py-2 text-sm outline-none focus:border-zinc-500 dark:border-white/[0.1] dark:bg-[#222222] dark:text-white dark:focus:border-slate-500"
          />
        </div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <Card key={s.id} className="flex flex-col rounded-2xl border border-zinc-200/80 bg-white p-5 sm:p-6 shadow-xs dark:border-white/[0.08] dark:bg-[#1a1a1a]">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="truncate font-bold text-zinc-800 dark:text-white text-base">{s.name || 'Emergency Shelter'}</div>
                    <div className="mt-1 flex items-start gap-1 line-clamp-2 text-xs leading-relaxed text-slate-500 dark:text-white">
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-slate-500 dark:text-white">
                  <span className="font-semibold text-zinc-800 dark:text-white mono">
                    {distance !== null ? `${distance.toFixed(1)} ${t('shelter.kmAway')}` : t('common.verified')}
                  </span>
                  {s.contactPhone && (
                    <a href={`tel:${s.contactPhone}`} className="flex items-center gap-1 text-zinc-600 hover:underline font-medium dark:text-white mono">
                ...
                <div className="mt-auto pt-4">
                  <div className="flex justify-between text-xs mb-1">
                    <span className="flex items-center gap-1 text-slate-500 dark:text-white">
                ...
                <div className="mt-3 flex flex-wrap gap-1">
                  {facilitiesList.map((f) => (
                    <span key={f} className="rounded px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-zinc-500 dark:bg-[#222222] dark:text-white mono">
          ...
          {filteredAndSorted.length === 0 && (
            <div className="col-span-full rounded-2xl border border-dashed border-zinc-200 p-12 text-center text-xs text-slate-500 dark:border-white/[0.08] dark:text-white">
