      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{t('bulletin.title')}</h1>
          </div>
        ...
        <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1 dark:border-slate-800 dark:bg-slate-900 shadow-xs">
      ...
      <div className="space-y-3">
        {filtered.map((a) => {
          ...
          return (
            <div
              key={a.id}
              className={`rounded-2xl border border-slate-200 bg-white p-5 shadow-xs transition dark:border-slate-800 dark:bg-slate-900 ${borderClass}`}
            >
              ...
              <p className="mt-2.5 text-xs leading-relaxed text-slate-600 dark:text-slate-300">{a.message}</p>

              {a.region && (
                <div className="mt-3 flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 border-t border-slate-100 pt-2.5 dark:border-slate-800">
                  <MapPin className="h-3.5 w-3.5 text-slate-400" />
                  <span className="font-semibold text-slate-700 dark:text-slate-300">Affected Area: {a.region}</span>
                </div>
              )}
            </div>
          )
