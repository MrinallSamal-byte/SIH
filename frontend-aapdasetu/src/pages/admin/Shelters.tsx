  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-xs dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-800 outline-none focus:border-slate-900 dark:border-white/[0.1] dark:bg-slate-950 dark:text-white"
          >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div>
                ...
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div>
                    ...
                    <div className="mt-0.5 flex items-start gap-1 text-xs text-slate-500 dark:text-white line-clamp-2">
                ...
                {/* Coordinates & Phone */}
                <div className="flex items-center justify-between text-xs text-slate-500 dark:text-white mt-2 border-t border-slate-100 pt-2 dark:border-slate-800">
                  <span className="mono text-[11px]">{s.latitude.toFixed(4)}°N, {s.longitude.toFixed(4)}°E</span>
                  {s.contactPhone && (
                    <span className="flex items-center gap-1 font-mono text-slate-700 dark:text-white">
                ...
                <div className="mt-4 space-y-1.5">
                  <div className="flex justify-between text-xs font-semibold">
                    <span className="text-slate-600 dark:text-white flex items-center gap-1">
                ...
                <div className="mt-3 flex flex-wrap gap-1">
                  {s.facilities.map((f) => (
                    <span
                      key={f}
                      className="rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:bg-slate-800 dark:text-white mono"
                    >
                      {f.replace(/_/g, ' ')}
                    </span>
                  ))}
                ...
                <div className="flex items-center justify-between gap-2 pt-1">
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="rounded-lg border border-slate-200 px-2 py-1 text-[11px] font-bold text-slate-700 hover:bg-slate-100 dark:border-white/[0.1] dark:text-white"
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-lg border border-slate-300 bg-white px-2.5 py-1 text-xs font-bold text-slate-700 hover:bg-slate-50 dark:border-white/[0.1] dark:bg-slate-800 dark:text-white cursor-pointer"
                    >
        ...
        {filtered.length === 0 && (
          <div className="col-span-full rounded-2xl border border-dashed border-slate-300 p-12 text-center text-xs text-slate-500 dark:border-slate-800 dark:text-white">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
          <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900">
            ...
            <form onSubmit={handleSave} className="mt-4 space-y-4 text-left">
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-white mb-1 mono uppercase">
                  Shelter Name *
                </label>
                <input
                  type="text"
                  ...
                  className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2 text-sm text-slate-900 outline-none focus:border-slate-900 dark:border-white/[0.1] dark:bg-slate-800 dark:text-white"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2 text-sm font-mono text-slate-900 outline-none focus:border-slate-900 dark:border-white/[0.1] dark:bg-slate-800 dark:text-white"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-white mb-1 mono uppercase">
              ...
              <div>
                <button
                  ...
                  className="text-xs font-bold text-slate-900 hover:underline dark:text-white cursor-pointer"
                >
              ...
              {/* Status selection */}
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-white mb-1.5 mono uppercase">
                  Operation Status
                </label>
                    ...
                    <button
                      ...
                      className={`rounded-xl border p-2.5 text-xs font-bold transition cursor-pointer ${
                        status === st.val
                          ...
                          : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50 dark:border-white/[0.1] dark:bg-slate-800 dark:text-white'
                      }`}
                    >
                      {st.label}
                    </button>
                    ...
                    <label
                      ...
                      className={`flex items-center gap-2 rounded-xl border p-2.5 text-xs font-medium cursor-pointer transition ${
                        facilities.includes(f.id)
                          ...
                          : 'border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100 dark:border-white/[0.1] dark:bg-slate-800 dark:text-white'
                      }`}
                    >
                      <input
                        type="checkbox"
              ...
              <div className="flex justify-end gap-2 border-t border-slate-100 pt-4 dark:border-slate-800">
                <button
                  type="button"
                  onClick={() => setModalOpen(false)}
                  className="rounded-xl border border-slate-300 px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 dark:border-white/[0.1] dark:bg-slate-800 dark:text-white"
                >
                  Cancel
                </button>
                <button
